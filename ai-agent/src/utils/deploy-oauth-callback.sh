...
ZIP_PATH="$BUILD_DIR/oauth2_callback.zip"

echo "=========================================="
echo "OAuth2 Callback Lambda Deployment"
echo "=========================================="
echo "Region:            $REGION"
echo "Lambda Name:       $OAUTH_LAMBDA_NAME"
echo "OAuth Table:       $OAUTH_TABLE"
...
if ! aws lambda get-function-configuration \
  ...
  echo "ERROR: Lambda function '$OAUTH_LAMBDA_NAME' not found in region $REGION"
  exit 1
fi
...
# Copy Lambda code
echo "  - Copying oauth2_callback_server.py..."
cp "$LAMBDA_DIR/oauth2_callback_server.py" "$BUILD_DIR/package/"
...
aws lambda update-function-configuration \
  --region "$REGION" \
  --function-name "$OAUTH_LAMBDA_NAME" \
  --handler "oauth2_callback_server.handler" \
  --environment "Variables={OAUTH_CALLBACK_TABLE=$OAUTH_TABLE,ALLOW_ORIGIN=$ALLOW_ORIGIN}" \
  --output json \
